...
const oneSignalRequest = async (method, path, body = null) => {
  const options = {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Basic ${process.env.ONE_SIGNAL_REST_API_KEY}`, // v2 format
    },
  };
  if (body) options.body = JSON.stringify(body);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: JSON.stringify(error.data) });
    }
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      let filter = {};
      if (req.user.role === "admin") {
        filter.adminId = req.user._id; // Regular users see only their notifications
      }

      console.log("App ID:", process.env.ONE_SIGNAL_APP_ID);
      const notifications = await Notification.find(filter).sort({ _id: -1 });
      res.json({
        success: true,
