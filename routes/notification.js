...
// for testing noti on onesignal on vercel
...
const Notification = require('../model/notification');
const OneSignal = require('@onesignal/node-onesignal');
const { on } = require('../model/category');

console.log("App ID:", process.env.ONE_SIGNAL_APP_ID)
// Configuration - Simplified for Vercel environment
const configuration = OneSignal.createConfiguration({
    authMethods: {
        app_key: {
            tokenProvider: {
...
router.post('/send-notification', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Notification sent successfully', data: { id: notificationId } });
    } catch (error) {
...
router.get('/track-notification/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Stats retrieved', data: result });
    } catch (error) {
