...
router.post('/register', asyncHandler(async (req, res) => {
  ...
  res.status(201).json({
    success: true,
    message: "Registration successful! A verification email has been sent. Please verify your email to access your account.",
    data: null
  });
}));
...
if(requiredRole=='admin'&& user.role !=='admin'){
  ...
  const { password: _, verificationToken: __, verificationTokenExpires: ___, ...userData } = user.toObject();
...
router.post('/guest', asyncHandler(async (req, res) => {
  ...
  const { password: _, verificationToken: __, verificationTokenExpires: ___, ...userData } = guestUser.toObject();
...
router.get('/verify-email/:token', asyncHandler(async (req, res) => {
  ...
  if (!user) {
    return res.status(400).send(getHtmlPage(
      "Verification Failed", 
  ...
  res.status(200).send(getHtmlPage(
    "Email Verified", 
